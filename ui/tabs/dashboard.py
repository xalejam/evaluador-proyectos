...
def _load_projects_from_db() -> list[dict]:
    ...
    with get_sqlite_conn(PV_DB_PATH) as conn:
        try:
            rows = conn.execute(
                "SELECT * FROM projects WHERE status IS NOT NULL ORDER BY updated_at DESC"
            ).fetchall()
...
def render_dashboard():
    ...
    with col_factors1:
        ...
        complexity_levels = [p.get('implementation_complexity') for p in projects if p.get('implementation_complexity') is not None]
    ...
    if len(trackings) > 0:
        ...
        if len(trackings) > 1:
            ...
            for _, tracking in trackings.iterrows():
                project = next((p for p in projects if (p.get('project_id') or p.get('id')) == tracking['project_id']), None)
                if project:
    ...
    if st.button(t("dashboard_export_excel_btn")):
        st.info("La exportación a Excel estará disponible una vez que se complete la migración completa a base de datos.")
