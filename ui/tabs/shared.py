...
class ExcelSharePointManager:
    ...
    def __init__(self, file_url: str, username: str = None, password: str = None):
        ...
        self.project_columns = [
            'id', 'name', 'description', 'created_date', 'status', 'last_tracking_update',
            'country', 'owner',
            ...
            'viability_score', 'priority', 'monthly_savings', 'annual_savings',
            'payback_period_months', 'roi_first_year', 'recommendation',
        ...
        self.tracking_columns = [
            'id', 'project_id', 'tracking_date', 'months_tracked',
    ...
    def add_project(self, project_data: dict) -> str:
        ...
        with self._get_connection() as conn:
            conn.execute(
                f"INSERT INTO projects ({columns_str}) VALUES ({placeholders})",
                tuple(record.values())
            )
    ...
    def add_tracking(self, tracking_data: dict) -> str:
        ...
        with self._get_connection() as conn:
            conn.execute(
                f"INSERT INTO tracking ({columns_str}) VALUES ({placeholders})",
                tuple(record.values())
            )
            conn.execute(
                "UPDATE projects SET status = ? WHERE id = ?",
                ('implemented', project_id)
            )
    ...
    def get_project_tracking(self, project_id: str) -> list:
        ...
        with self._get_connection() as conn:
            df = pd.read_sql_query(
                "SELECT * FROM tracking WHERE project_id = ? ORDER BY tracking_date ASC",
                conn,
                params=(project_id,)
            )
            return df.to_dict('records')
...
class ProjectViabilityCalculator:
    """Calculadora de viabilidad con NUEVO sistema de scoring"""
    ...
    def calculate_viability(self, project_data: dict) -> dict:
        """Calcula viabilidad del proyecto con NUEVO sistema (SIN factor económico)"""
        
        # =============================================================================
        # CÁLCULOS FINANCIEROS (Solo informativos, NO afectan score)
        # =============================================================================
        total_hours_per_month = (project_data['current_time_per_task'] * 
                               project_data['tasks_per_month'] * 
                               project_data['staff_count'])
        
        time_saved = (project_data['current_time_per_task'] * 
                     project_data['time_reduction_percent'] / 100)
        
        hours_saved_per_month = (time_saved * project_data['tasks_per_month'] * 
                               project_data['staff_count'])
        
        monthly_savings = hours_saved_per_month * project_data['avg_salary_per_hour']
        annual_savings = monthly_savings * 12
        
        initial_development_cost = (project_data['development_hours'] * 
                                  project_data['development_cost_per_hour'])
        ...
        if initial_development_cost > 0:
            roi_first_year = ((net_annual_benefit - initial_development_cost) / 
                            initial_development_cost) * 100
        else:
            roi_first_year = 0
        
        # =============================================================================
        # NUEVO SISTEMA DE SCORING (100 puntos - SIN factor económico)
        # =============================================================================
        viability_score = 0
        
        # 1. FACTOR DE IMPACTO (35 puntos máximo)
        ...
        if time_reduction >= 70:
            ...
            viability_score += max(0, time_reduction * 0.5)  # Hasta 2.5 puntos para <5%
        
        # 2. FACTOR DE RIESGO (30 puntos máximo - INVERSO)
        risk_level = project_data['risk_level']
        risk_points = {1: 30, 2: 24, 3: 18, 4: 12, 5: 6}
        viability_score += risk_points.get(risk_level, 6)
        
        # 3. FACTOR DE COMPLEJIDAD (35 puntos máximo - INVERSO)
        ...
        # =============================================================================
        # NUEVA LÓGICA DE RECOMENDACIÓN (basada en factibilidad e impacto)
        # =============================================================================
        if viability_score >= 80:
            recommendation = t('recommendation_80_100')
            priority = t('priority_high')
        elif viability_score >= 60:
            recommendation = t('recommendation_60_79')
            priority = t('priority_medium_high')
        elif viability_score >= 40:
            recommendation = t('recommendation_40_59')
            priority = t('priority_medium')
        else:
            recommendation = t('recommendation_0_39')
        ...
        return {
            ...
            # Nuevo scoring (factibilidad + impacto)
    ...
    def calculate_tracking_results(self, project_data: dict, tracking_data: dict) -> dict:
        """Calcula resultados de seguimiento"""
        ...
        actual_savings_per_task = (project_data['current_time_per_task'] - 
                                 tracking_data['actual_time_per_task'])
        ...
        if project_data['current_time_per_task'] > 0:
            actual_time_reduction_percent = ((project_data['current_time_per_task'] - 
                                           tracking_data['actual_time_per_task']) / 
                                          project_data['current_time_per_task']) * 100
        else:
            actual_time_reduction_percent = 0
        
        actual_monthly_savings = (actual_savings_per_task * 
        ...
        # Performance score
        if efficiency_ratio >= 1.2: performance_score = 100
        elif efficiency_ratio >= 1: performance_score = 90
        elif efficiency_ratio >= 0.8: performance_score = 75
        elif efficiency_ratio >= 0.6: performance_score = 60
        elif efficiency_ratio >= 0.4: performance_score = 40
        else: performance_score = 20
        
        # Ajustes por adopción y satisfacción
